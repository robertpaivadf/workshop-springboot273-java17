...
@RestController
@RequestMapping(value = "/products") // lá no postman no GET pode se usar : http://localhost:8080/users
public class ProductResource {
//Camada de Recurso que são os controladores REST --> Serviços(services) --> Dados (repositories)

	@Autowired //para o spring já fazer a injeção de dependência --> ProductResource que depende de ProductService
	private ProductService service;
	
	@GetMapping
	public ResponseEntity<List<Product>> findAll() { //ResponseEntity é um tipo específico do Spring para retornar reseposta de requisiçoes web
	...
	public ResponseEntity<Product> findById(@PathVariable Long id) { //Use @PathVariable pro spring entender que haverá parâmentros vindo de requisição
