...
@RestController
@RequestMapping(value = "/orders") // lá no postman no GET pode se usar : http://localhost:8080/users
public class OrderResource {
//Camada de Recurso que são os controladores REST --> Serviços(services) --> Dados (repositories)

	@Autowired //para o spring já fazer a injeção de dependência --> OrderResource que depende de OrderService
	private OrderService service;
	
	@GetMapping
	public ResponseEntity<List<Order>> findAll() { //ResponseEntity é um tipo específico do Spring para retornar reseposta de requisiçoes web
	...
	public ResponseEntity<Order> findById(@PathVariable Long id) { //Use @PathVariable pro spring entender que haverá parâmentros vindo de requisição
